...
class PreparedContentServiceBean @Autowired constructor(
	...
	override fun generateHtmlForTemplate(
		...
		when (dynamicContent) {
			is DynamicContent.Json -> {
				content.placeholderValues[dynamicContent.key] =
					HtmlTable.fromJson(dynamicContent.json, cTemplate.cssClasses.split(" "))
			}

			is DynamicContent.ListType -> {
				content.placeholderValues[dynamicContent.key] =
					HtmlTable.toHtml(dynamicContent.headers, dynamicContent.list, cTemplate.cssClasses)
			}
		}
	...
	override fun preparePageContent(contentId: Long?, title: String, content: String): PreparedContent {
		val preparedContent = contentId?.let {
			this.preparedContentRepository.find(it)
				.orElseThrow { ExceptionUtil.notFound(PreparedContent::class.java, it) }
		}
		val template: ContentTemplate = (preparedContent?.template?.let {
			it.content = content
			it
		} ?: ContentTemplate().apply {
			this.type = TemplateTypes.PAGE
			this.title = title
			this.code = UUID.randomUUID().toString()
			this.content = content
			this.cssClasses = ""
			this.placeholders = setOf()
		}).let {
			this.templateRepository.save(it)
		}

		return (preparedContent ?: PreparedContent().apply {
			this.title = title
			this.template = template
			this.placeholderValues = mutableMapOf()
			this.status = ContentStatuses.FINALIZED
		}).let {
			save(it)
		}
	}
